...
public class JoinOptimizer {
    ...
    public double estimateJoinCost(LogicalJoinNode j, int card1, int card2,
        ...
        if (j instanceof LogicalSubplanJoinNode) {
            ...
            // i don't believe I did (did we even implement a join alg?)
            return cost1 + (card1 * cost2) + (card1 * card2); // i THINK???? assuming pushed down predicates and such?
            // cost of scanning table1 once + cost of scanning table2 for each tuple fitting pred in table1 + cost to join each tuple fitting pred from each table?
        }
    }
    ...
    public static int estimateTableJoinCardinality(Predicate.Op joinOp,
        ...
        if (joinOp.equals(Predicate.Op.EQUALS) || joinOp.equals(Predicate.Op.LIKE)) { // i think these are the same....?
            if (!t1pkey && !t2pkey) { // if neither side is a primary key
                return (card1 >= card2) ? card1 : card2; // made up -- higher cardinality of the two
            } else if (!t1pkey) { // if right side isn't pkey but left is
                return card2; // eq join with only one pkey field
            } else { // if left side isn't pkey but right is
                return card1; // eq join with only one pkey field
            }
        }
        else { // range scan
            return (int) (0.3 * (card1 * card2)); // maybe this should be math.ceil? can't return half a tuple -- estimate anyway
        }
    }
    ...
    public Vector<LogicalJoinNode> orderJoins(
        ...
        int joinsSize = this.joins.size();

        for (int i = 1; i <= joinsSize; i++) {
            // internal loop
            for (Set<LogicalJoinNode> s : enumerateSubsets(this.joins, i)) { // for each loop enumerating thru subsets
                CostCard winningPlan = null;
                // now we have to actually go through each logical join node
                for (LogicalJoinNode j : s) {
                    double currCost = (winningPlan != null) ? winningPlan.cost : (double) Double.MAX_VALUE;

                    CostCard subplan = this.computeCostAndCardOfSubplan(stats, filterSelectivities, j, s, currCost, planCache); //costCard for subpaln

                    // check if subplan 'clobbers' winning plan :)
                    winningPlan = (subplan != null && (winningPlan == null || winningPlan.cost > subplan.cost)) ? subplan : winningPlan;

                }
                if (winningPlan != null) planCache.addPlan(s, winningPlan.cost, winningPlan.card, winningPlan.plan);
            }

        }

        HashSet<LogicalJoinNode> joinsSet = new HashSet<>(this.joins);
        Vector<LogicalJoinNode> order = planCache.getOrder(joinsSet);

        if (explain) printJoins(order, planCache, stats, filterSelectivities);
